...
import EventsDashboard2 from "@/components/dashboard/EventsDashboard2";
...
export default async function DashboardPage({isNoti}) {
  ...
  return (
    <DashboardShell>
      {(isNoti)?
             <DashboardHeader heading="Logs" text="Overview of all your logs">
      ...
      <div>
        {channels.length === 0 ? (
          ...
          // Use a valid JSX expression for rendering based on isNoti
          isNoti ? (
            <EventsDashboard2 events={events} eventStats={eventStats} />
          ) : (
